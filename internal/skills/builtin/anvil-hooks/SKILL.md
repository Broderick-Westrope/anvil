...
Hooks are user-defined commands in `anvil.json` that fire at specific points
...
3. Add the entry under `hooks.PreToolUse` in `anvil.json` with the right matcher.
...
- Non-zero exit codes other than 2/49 are logged but don't block — check Anvil logs.
...
Anvil also accepts Claude Code's `hookSpecificOutput` envelope. One intentional
divergence: Anvil treats `updated_input` as shallow-merge, Claude Code replaces.
