...
var loginCmd = &cobra.Command{
	Aliases: []string{"auth"},
	Use:     "login [platform]",
	Short:   "Login Anvil to a platform",
	Long: `Login Anvil to a specified platform.
...
func loginHyper(c *client.Client, wsID string, force bool) error {
	...
	if err := c.SetProviderAPIKey(ctx, wsID, config.ScopeGlobal, "hyper", token); err != nil {
		return err
	}
...
func loginCopilot(c *client.Client, wsID string, force bool) error {
	...
	default:
		...
		_ = clipboard.WriteAll(dc.UserCode)
		fmt.Println()
		fmt.Println("The following code should be on clipboard already:")
		fmt.Println()
	...
	if err := c.SetProviderAPIKey(loginCtx, wsID, config.ScopeGlobal, "copilot", token); err != nil {
		return err
	}
