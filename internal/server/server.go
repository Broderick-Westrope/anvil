...
import (
	...
	_ "github.com/Broderick-Westrope/anvil/internal/swagger"
...
func DefaultHost() string {
	...
	if err == nil && usr.Uid != "" {
		sock = fmt.Sprintf("anvil-%s.sock", usr.Uid)
	}
...
// Server represents an Anvil server bound to a specific address.
type Server struct {
...
func NewServer(cfg *config.ConfigStore, network, address string) *Server {
	...
	mux.HandleFunc("PUT /v1/workspaces/{id}/sessions/{sid}/pin", c.handlePutWorkspaceSessionPin)
