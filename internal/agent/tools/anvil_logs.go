...
const AnvilLogsToolName = "anvil_logs"

//go:embed anvil_logs.md.tpl
var anvilLogsDescriptionTmpl []byte

var anvilLogsDescriptionTpl = template.Must(
	template.New("anvilLogsDescription").
		Parse(string(anvilLogsDescriptionTmpl)),
)

type anvilLogsDescriptionData struct {
	DefaultLines int
	MaxLines     int
}

func anvilLogsDescription() string {
	return renderTemplate(anvilLogsDescriptionTpl, anvilLogsDescriptionData{
...
type AnvilLogsParams struct {
	Lines int `json:"lines,omitempty" description:"Number of recent log entries to return (default 50, max 100)"`
}

func NewAnvilLogsTool(logFile string) fantasy.AgentTool {
	return fantasy.NewAgentTool(
		AnvilLogsToolName,
		anvilLogsDescription(),
		func(ctx context.Context, params AnvilLogsParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			result := runAnvilLogs(logFile, params)
			return fantasy.NewTextResponse(result), nil
		},
	)
}

// runAnvilLogs reads and formats the last N log entries from the given file.
func runAnvilLogs(logFile string, params AnvilLogsParams) string {
