...
// lets Anvil reap a server's descendant processes (e.g. signal-cli launched by
...
func TestCreateTransport_StdioProcessGroup(t *testing.T) {
	...
	tr, err := createTransport(t.Context(), "stdio-test", m, shellResolverWithPath(t, nil), nil)
