...
// AnvilEnvMarkers returns a fresh slice of the environment variables that
// Anvil unconditionally sets on every shell it spawns — both the interactive
...
func NewShell(opts *Options) *Shell {
	...
	// Allow tools to detect execution by Anvil.
	env = append(env, AnvilEnvMarkers()...)
