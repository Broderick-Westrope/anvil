...
func TestBuildEnv(t *testing.T) {
	...
	require.Equal(t, EventPreToolUse, envMap["ANVIL_EVENT"])
	require.Equal(t, "bash", envMap["ANVIL_TOOL_NAME"])
	require.Equal(t, "sess-1", envMap["ANVIL_SESSION_ID"])
	require.Equal(t, "/work", envMap["ANVIL_CWD"])
	require.Equal(t, "/project", envMap["ANVIL_PROJECT_DIR"])
	require.Equal(t, "ls", envMap["ANVIL_TOOL_INPUT_COMMAND"])
	require.Equal(t, "/tmp/f.txt", envMap["ANVIL_TOOL_INPUT_FILE_PATH"])

	// Shared Anvil markers must be present so hook-authored scripts can
	// detect they're running under Anvil the same way bash-tool-invoked
	// scripts can.
	require.Equal(t, "1", envMap["ANVIL"])
	require.Equal(t, "anvil", envMap["AGENT"])
	require.Equal(t, "anvil", envMap["AI_AGENT"])
}
...
func TestRunnerEnvVarsPropagated(t *testing.T) {
	t.Parallel()
	hookCfg := config.HookConfig{
		Command: `printf '{"decision":"allow","context":"%s"}' "$ANVIL_TOOL_NAME"`,
	}
	r := NewRunner([]config.HookConfig{hookCfg}, t.TempDir(), t.TempDir())
	result, err := r.Run(context.Background(), EventPreToolUse, "sess", "bash", `{}`)
	require.NoError(t, err)
...
func TestParseStdoutClaudeCodeFormat(t *testing.T) {
	...
	t.Run("anvil format still works", func(t *testing.T) {
		t.Parallel()
