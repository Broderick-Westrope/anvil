...
func TestAnvilIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .anvilignore file that ignores .log files
	require.NoError(t, os.WriteFile(".anvilignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .anvilignore file
	anvilignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".anvilignore"), []byte(anvilignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .anvilignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .anvilignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .anvilignore in subdir that ignores normal_nested
	subAnvilignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".anvilignore"), []byte(subAnvilignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .anvilignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .anvilignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .anvilignore")
