...
func TestProviders_Integration_WithMockClients(t *testing.T) {
	...
	// Create fresh syncer for this test.
	testCatwalkSyncer := &catwalkSync{}
	...
	catwalkPath := tmpDir + "/anvil/providers.json"

	testCatwalkSyncer.Init(mockCatwalkClient, catwalkPath, true)

	// Get providers from syncer.
...
func TestProviders_Integration_WithCachedData(t *testing.T) {
	...
	require.NoError(t, os.MkdirAll(tmpDir+"/anvil", 0o755))
	...
	// Mock client that returns ErrNotModified.
	mockCatwalkClient := &mockCatwalkClient{
...
func TestCachePathFor(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected:    "/custom/data/anvil/providers.json",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expected != "" {
				...
				require.Contains(t, result, "anvil")
