...
// config file at path, mimicking what another Anvil instance would leave
...
// which lets a single test process faithfully simulate two Anvil instances:
...
func TestRefreshOAuthToken_InProcessSingleFlight(t *testing.T) {
	...
	configPath := filepath.Join(t.TempDir(), "anvil.json")
