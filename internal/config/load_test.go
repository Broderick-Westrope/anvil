...
func TestLookupConfigs_BoundedByProject(t *testing.T) {
	...
	globalDir := t.TempDir()
	t.Setenv("ANVIL_GLOBAL_CONFIG", globalDir)
	t.Setenv("ANVIL_GLOBAL_DATA", globalDir)

	t.Run("does not pick up anvil.json above non-git project", func(t *testing.T) {
		parent := t.TempDir()

		// anvil.json above the project must not be adopted.
		require.NoError(t, os.WriteFile(
			filepath.Join(parent, "anvil.json"),
			[]byte(`{}`),
			0o644,
		))
		...
		for _, p := range got {
			require.NotEqual(t, filepath.Join(parent, "anvil.json"), p)
		}
	})

	t.Run("does not climb out of git worktree to find anvil.json", func(t *testing.T) {
		...
		strayEval, err := filepath.EvalSymlinks(filepath.Join(parent, "anvil.json"))
		require.NoError(t, err)
		for _, p := range got {
			...
			require.NotEqual(t, strayEval, pEval, "must not adopt parent anvil.json")
		}
	})

	t.Run("picks up anvil.json inside the project", func(t *testing.T) {
		project := t.TempDir()
		local := filepath.Join(project, "anvil.json")
		...
		require.True(t, foundLocal, "expected project anvil.json to be in lookup result: %v", got)
	})
...
func TestConfig_setDefaults(t *testing.T) {
	...
	t.Run("does not climb out of git worktree to find .anvil", func(t *testing.T) {
		if _, err := exec.LookPath("git"); err != nil {
			t.Skip("git not available")
		}
...
func TestConfig_setupDefaultAgentsWithEveryReadOnlyToolDisabled(t *testing.T) {
	...
	cfg := &Config{
		Options: &Options{
			DisabledTools: []string{"glob", "grep", "ls", "lsp_call_hierarchy", "lsp_definition", "lsp_symbols", "sourcegraph", "view"},
		},
	}
...
func TestConfig_setDefaultsDisableDefaultProvidersEnvVar(t *testing.T) {
	t.Run("sets option from environment variable", func(t *testing.T) {
		t.Setenv("ANVIL_DISABLE_DEFAULT_PROVIDERS", "true")
