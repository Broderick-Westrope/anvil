...
func PushPopAnvilEnv() func() {
	var found []string
	for _, ev := range os.Environ() {
		if strings.HasPrefix(ev, "ANVIL_") {
			pair := strings.SplitN(ev, "=", 2)
			if len(pair) != 2 {
				continue
			}
			found = append(found, strings.TrimPrefix(pair[0], "ANVIL_"))
		}
	}
	...
	for _, ev := range found {
		os.Setenv(ev, os.Getenv("ANVIL_"+ev))
	}
...
func (c *Config) setDefaults(workingDir, dataDir string) {
	...
	if str, ok := os.LookupEnv("ANVIL_DISABLE_PROVIDER_AUTO_UPDATE"); ok {
		c.Options.DisableProviderAutoUpdate, _ = strconv.ParseBool(str)
	}

	if str, ok := os.LookupEnv("ANVIL_DISABLE_DEFAULT_PROVIDERS"); ok {
...
func GlobalConfig() string {
	if anvilGlobal := os.Getenv("ANVIL_GLOBAL_CONFIG"); anvilGlobal != "" {
		return filepath.Join(anvilGlobal, fmt.Sprintf("%s.json", appName))
	}
...
func GlobalCacheDir() string {
	if anvilCache := os.Getenv("ANVIL_CACHE_DIR"); anvilCache != "" {
		return anvilCache
	}
...
func GlobalConfigData() string {
	if anvilData := os.Getenv("ANVIL_GLOBAL_DATA"); anvilData != "" {
		return filepath.Join(anvilData, fmt.Sprintf("%s.json", appName))
	}
	...
	// for windows, it should be in `%LOCALAPPDATA%/anvil/`
	// for linux and macOS, it should be in `$HOME/.local/share/anvil/`
	if runtime.GOOS == "windows" {
		localAppData := cmp.Or(
			os.Getenv("LOCALAPPDATA"),
			filepath.Join(os.Getenv("USERPROFILE"), "AppData", "Local"),
...
// fallback keeps Anvil from silently adopting state files placed above
// the current project.
...
func GlobalSkillsDirs() []string {
	if anvilSkills := os.Getenv("ANVIL_SKILLS_DIR"); anvilSkills != "" {
		return []string{anvilSkills}
	}
...
// ProjectSkillsDir returns the default project directories for which Anvil
// will look for skills.
func ProjectSkillsDir(workingDir string) []string {
	return []string{
		filepath.Join(workingDir, ".agents/skills"),
		filepath.Join(workingDir, ".anvil/skills"),
