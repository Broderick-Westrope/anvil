...
- The `stats` subcommand could be a TUI similar to [codeburn](https://github.com/AgentSeal/codeburn) — interactive charts, heatmaps, and drill-downs instead of static HTML output.
- On quit, output the active session ID and a ready-to-paste resume command (like OpenCode does), e.g. `anvil -s abc123  # "Fix auth bug"`.
- Make it possible to rename sessions after creation — either manually via `anvil sessions rename <id> "new title"` or by regenerating the title from conversation context.
- When showing all sessions (`anvil sessions list --all` and the TUI modal toggle), display the `working_dir` for each session so users can distinguish sessions from different projects.
- The `ctrl+a` keybind for toggling local/all sessions in the TUI modal is not discoverable. Consider a better keybind or a visible toggle button in the dialog chrome.
