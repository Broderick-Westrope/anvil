...
the agent lifecycle, allowing you to both build on top of Anvil, customize
...
our **project-level** `anvil.json`. Relative paths like `./no-haskell.sh` work
here because the project root is your working directory. If you're configuring
a global hook (`~/.config/anvil/anvil.json`), use an absolute path instead.
...
# ($ANVIL_TOOL_INPUT_COMMAND) to grep and match on a regexp.
if echo "$ANVIL_TOOL_INPUT_COMMAND" | grep -qE '(^| )((ghc|cabal|stack)(\.exe)?)( |$)'; then
...
Hooks run through Anvil's embedded POSIX shell (`mvdan.cc/sh`) — the same
...
- **Permissive shebang fallback**: if the absolute path in a shebang
  doesn't exist (e.g. `#!/bin/bash` on Windows), Anvil falls back to a
  ...
  "no opinion".
- **Environment**: every hook sees `ANVIL=1`, `AGENT=anvil`, and
  `AI_AGENT=anvil` on top of the `ANVIL_*` hook-specific variables. These
  ...
  both contexts.
- **Timeout behavior**: when a hook exceeds its timeout, Anvil cancels the
  context and waits a short grace period (~1s) for the interpreter to
  yield. If the hook still hasn't returned, Anvil abandons it, logs a
...
Hooks can be added to your `anvil.json` (or `.anvil.json`) at both the global
...
> work fine in project-level `anvil.json` because the project root is also
> your working directory. For **global** config (`~/.config/anvil/`),
> however, you must use either an absolute path or an inline command:
>
> ```jsonc
> // Global ~/.config/anvil/anvil.json
> {
>   "hooks": {
>     "PreToolUse": [
>       {
>         "command": "/home/you/.config/anvil/hooks/no-haskell.sh"
...
3. Runs all matching hooks **in parallel** through Anvil's embedded POSIX
...
5. Applies the result **before** permission checks. If the aggregated decision
   ...
   for it. If it's `allow`, Anvil treats that as affirmative pre-approval and
...
however you'll incur some additional overhead as Anvil will still parse and
run each hook.
...
| ---------------------------- | ---------------------------------------------- |
| `ANVIL`                      | Always `1` when running under Anvil.           |
| `AGENT`                      | Always `anvil`.                                |
| `AI_AGENT`                   | Always `anvil`.                                |
| `ANVIL_EVENT`                | The hook event name (e.g. `PreToolUse`).       |
| `ANVIL_TOOL_NAME`            | The tool being called (e.g. `bash`).           |
| `ANVIL_SESSION_ID`           | Current session ID.                            |
| `ANVIL_CWD`                  | Working directory.                             |
| `ANVIL_PROJECT_DIR`          | Project root directory.                        |
| `ANVIL_TOOL_INPUT_COMMAND`   | For `bash` calls: the shell command being run. |
| `ANVIL_TOOL_INPUT_FILE_PATH` | For file tools: the target file path.          |
...
Hooks communicate back to Anvil via **exit code** and `stdout`/`stderr`. The
...
deterministic based on the order hooks appear in `anvil.json`.
...
If a hook exceeds its timeout, Anvil cancels its context and treats the
...
if echo "$ANVIL_TOOL_INPUT_COMMAND" | grep -qE 'rm\s+-(rf|fr)\s+/'; then
...
returns `decision: "allow"`, which tells Anvil to pre-approve the call:
...
case "$ANVIL_TOOL_INPUT_COMMAND" in
...
if [[ "$ANVIL_TOOL_INPUT_FILE_PATH" == *.go ]]; then
...
```jsonc
{ "command": "echo \"$(date -Iseconds) $ANVIL_TOOL_NAME\" >> ./tools.log" }
```
...
Anvil hooks are broadly compatible with [Claude Code
...
hooks run under Anvil unchanged. This document covers the Anvil-specific API
only — anything not documented here isn't guaranteed to work.

One intentional divergence: Anvil treats `updated_input` as a shallow-merge
...
4. `decision` precedence: `deny` > `allow` > `null`. First deny determines the
   ...
   is `allow`, Anvil pre-approves the tool call and skips the permission
